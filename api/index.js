...
async function addScore(userId, delta, reason) {
  try {
    const p = getPool();
    const [rows] = await p.query('SELECT score, details FROM user_activities WHERE userId = ?', [userId]);
    if (rows.length === 0) {
...
async function handleRequest(req, res, method, path, query, body) {
  ...
  if (method === 'POST' && path === '/api/register') {
    const [exists] = await p.query('SELECT _id FROM profiles WHERE username = ? LIMIT 1', [body.username]);
    if (exists.length > 0) {
    ...
    const [rows] = await p.query('SELECT * FROM profiles WHERE _id = ?', [id]);
    return sendJson(res, 200, { success: true, user: rows[0] });
  }

  // ---------- 公告 ----------
  if (method === 'GET' && path === '/api/notices') {
    const [rows] = await p.query('SELECT * FROM notices ORDER BY is_pinned DESC, created_at DESC');
  ...
  if (method === 'POST' && path === '/api/notices') {
    ...
    const [rows] = await p.query('SELECT * FROM notices WHERE _id = ?', [id]);
  ...
  if (method === 'PUT' && path.startsWith('/api/notices/')) {
    ...
    vals.push(id);
    await p.query('UPDATE notices SET ' + sets.join(', ') + ' WHERE _id = ?', vals);
    return sendJson(res, 200, { success: true });
  }

  if (method === 'DELETE' && path.startsWith('/api/notices/')) {
    const id = path.split('/')[3];
    await p.query('DELETE FROM notices WHERE _id = ?', [id]);
    await p.query('DELETE FROM notice_reads WHERE noticeId = ?', [id]);
    return sendJson(res, 200, { success: true });
  }

  if (method === 'POST' && path.match(/^\/api\/notices\/[^/]+\/read$/)) {
    const id = path.split('/')[3];
    const [exists] = await p.query('SELECT _id FROM notice_reads WHERE noticeId = ? AND userId = ? LIMIT 1', [id, body.userId]);
    if (exists.length === 0) {
      await p.query('INSERT INTO notice_reads (_id, noticeId, userId) VALUES (?,?,?)', [uuid(), id, body.userId]);
    }
    return sendJson(res, 200, { success: true });
  }
  ...
  if (method === 'GET' && path.match(/^\/api\/notices\/[^/]+\/read$/)) {
    const id = path.split('/')[3];
    const [rows] = await p.query('SELECT _id FROM notice_reads WHERE noticeId = ? AND userId = ? LIMIT 1', [id, query.userId]);
    return sendJson(res, 200, { success: true, read: rows.length > 0 });
  }

  // ---------- 投票 ----------
  if (method === 'GET' && path === '/api/polls') {
    const [rows] = await p.query('SELECT * FROM polls ORDER BY created_at DESC');
    return sendJson(res, 200, { success: true, data: rows });
  }
  if (method === 'POST' && path === '/api/polls') {
    ...
    const [rows] = await p.query('SELECT * FROM polls WHERE _id = ?', [id]);
    return sendJson(res, 200, { success: true, data: rows[0] });
  }

  if (method === 'PUT' && path.match(/^\/api\/polls\/[^/]+\/end$/)) {
    const id = path.split('/')[3];
    await p.query('UPDATE polls SET is_active = 0 WHERE _id = ?', [id]);
    return sendJson(res, 200, { success: true });
  }

  if (method === 'DELETE' && path.startsWith('/api/polls/') && path.split('/').length === 4) {
    const id = path.split('/')[3];
    await p.query('DELETE FROM polls WHERE _id = ?', [id]);
    await p.query('DELETE FROM vote_records WHERE pollId = ?', [id]);
    return sendJson(res, 200, { success: true });
  }

  if (method === 'POST' && path.match(/^\/api\/polls\/[^/]+\/vote$/)) {
    const id = path.split('/')[3];
    const [exists] = await p.query('SELECT _id FROM vote_records WHERE pollId = ? AND userId = ? LIMIT 1', [id, body.userId]);
    if (exists.length > 0) {
      return sendJson(res, 200, { success: false, message: '您已投过票' });
    }
    await p.query('INSERT INTO vote_records (_id, pollId, userId, option_index) VALUES (?,?,?,?)', [uuid(), id, body.userId, body.optionIndex]);
  ...
  if (method === 'GET' && path.match(/^\/api\/polls\/[^/]+\/voted$/)) {
    const id = path.split('/')[3];
    const [rows] = await p.query('SELECT option_index FROM vote_records WHERE pollId = ? AND userId = ? LIMIT 1', [id, query.userId]);
  ...
  if (method === 'GET' && path.match(/^\/api\/polls\/[^/]+\/records$/)) {
    const id = path.split('/')[3];
    const [rows] = await p.query('SELECT option_index FROM vote_records WHERE pollId = ?', [id]);
    return sendJson(res, 200, { success: true, data: rows });
  }

  // ---------- 任务 ----------
  if (method === 'GET' && path === '/api/tasks') {
    const [rows] = await p.query('SELECT * FROM tasks ORDER BY deadline ASC');
    return sendJson(res, 200, { success: true, data: rows });
  }
  if (method === 'POST' && path === '/api/tasks') {
    ...
    const [rows] = await p.query('SELECT * FROM tasks WHERE _id = ?', [id]);
    return sendJson(res, 200, { success: true, data: rows[0] });
  }
  ...
  if (method === 'PUT' && path.startsWith('/api/tasks/') && path.split('/').length === 4) {
    ...
    if (sets.length === 0) return sendJson(res, 200, { success: true });
    vals.push(id);
    await p.query('UPDATE tasks SET ' + sets.join(', ') + ' WHERE _id = ?', vals);
    return sendJson(res, 200, { success: true });
  }

  if (method === 'DELETE' && path.startsWith('/api/tasks/') && path.split('/').length === 4) {
    const id = path.split('/')[3];
    await p.query('DELETE FROM tasks WHERE _id = ?', [id]);
    return sendJson(res, 200, { success: true });
  }
  ...
  if (method === 'POST' && path === '/api/moments') {
    ...
    const [rows] = await p.query('SELECT * FROM moments WHERE _id = ?', [id]);
    return sendJson(res, 200, { success: true, data: rows[0] });
  }

  if (method === 'DELETE' && path.startsWith('/api/moments/') && path.split('/').length === 4) {
    const id = path.split('/')[3];
    await p.query('DELETE FROM moments WHERE _id = ?', [id]);
    await p.query('DELETE FROM moment_likes WHERE momentId = ?', [id]);
    return sendJson(res, 200, { success: true });
  }

  if (method === 'POST' && path.match(/^\/api\/moments\/[^/]+\/like$/)) {
    const id = path.split('/')[3];
    const [exists] = await p.query('SELECT _id FROM moment_likes WHERE momentId = ? AND userId = ? LIMIT 1', [id, body.userId]);
    if (exists.length > 0) {
      await p.query('DELETE FROM moment_likes WHERE momentId = ? AND userId = ?', [id, body.userId]);
      await p.query('UPDATE moments SET likes = likes - 1 WHERE _id = ? AND likes > 0', [id]);
      return sendJson(res, 200, { success: true, liked: false });
    } else {
      await p.query('INSERT INTO moment_likes (_id, momentId, userId) VALUES (?,?,?)', [uuid(), id, body.userId]);
      await p.query('UPDATE moments SET likes = likes + 1 WHERE _id = ?', [id]);
  ...
  if (method === 'GET' && path.match(/^\/api\/moments\/[^/]+\/liked$/)) {
    const id = path.split('/')[3];
    const [rows] = await p.query('SELECT _id FROM moment_likes WHERE momentId = ? AND userId = ? LIMIT 1', [id, query.userId]);
  ...
  if (method === 'GET' && path === '/api/checkins') {
    ...
    const [rows] = await p.query('SELECT * FROM check_ins WHERE userId = ? AND date >= ? AND date < ? ORDER BY date ASC', [userId, start, end]);
    return sendJson(res, 200, { success: true, data: rows });
  }
  if (method === 'POST' && path === '/api/checkins') {
    const userId = body.userId;
    const today = todayStr();
    const [todayRows] = await p.query('SELECT _id FROM check_ins WHERE userId = ? AND date = ? LIMIT 1', [userId, today]);
    if (todayRows.length > 0) {
      return sendJson(res, 200, { success: false, message: '今日已签到' });
    }
    const yesterday = new Date(Date.now() - 86400000).toISOString().slice(0, 10);
    const [yRows] = await p.query('SELECT streak FROM check_ins WHERE userId = ? AND date = ? LIMIT 1', [userId, yesterday]);
    const streak = yRows.length > 0 ? (yRows[0].streak + 1) : 1;
    await p.query('INSERT INTO check_ins (_id, userId, date, streak) VALUES (?,?,?,?)', [uuid(), userId, today, streak]);
  ...
  if (method === 'PUT' && path.startsWith('/api/profiles/') && path.split('/').length === 4) {
    ...
    if (sets.length === 0) return sendJson(res, 200, { success: true });
    vals.push(id);
    await p.query('UPDATE profiles SET ' + sets.join(', ') + ' WHERE _id = ?', vals);
    const [rows] = await p.query('SELECT * FROM profiles WHERE _id = ?', [id]);
    return sendJson(res, 200, { success: true, data: rows[0] });
  }

  if (method === 'GET' && path === '/api/profiles') {
    const [rows] = await p.query('SELECT * FROM profiles ORDER BY created_at DESC');
    return sendJson(res, 200, { success: true, data: rows });
  }

  // ---------- 活跃度 ----------
  if (method === 'GET' && path === '/api/user-activities') {
    const [rows] = await p.query('SELECT * FROM user_activities WHERE userId = ? LIMIT 1', [query.userId]);
  ...
  if (method === 'GET' && path.startsWith('/api/users/') && path.split('/').length === 4) {
    const id = path.split('/')[3];
    const [uRows] = await p.query('SELECT * FROM profiles WHERE _id = ?', [id]);
    if (uRows.length === 0) return sendJson(res, 200, { success: false, message: '用户不存在' });
    const [aRows] = await p.query('SELECT * FROM user_activities WHERE userId = ? LIMIT 1', [id]);
    const [cRows] = await p.query('SELECT * FROM check_ins WHERE userId = ? ORDER BY date DESC', [id]);
    const [mRows] = await p.query('SELECT * FROM moments WHERE authorId = ? ORDER BY created_at DESC', [id]);
    return sendJson(res, 200, {
      success: true,
  ...
  if (method === 'PUT' && path.match(/^\/api\/users\/[^/]+\/ban$/)) {
    ...
    await p.query('UPDATE profiles SET is_banned = 1, ban_expire_at = ? WHERE _id = ?', [expire, id]);
    return sendJson(res, 200, { success: true, ban_expire_at: expire });
  }
  if (method === 'PUT' && path.match(/^\/api\/users\/[^/]+\/unban$/)) {
    const id = path.split('/')[3];
    await p.query('UPDATE profiles SET is_banned = 0, ban_expire_at = NULL WHERE _id = ?', [id]);
    return sendJson(res, 200, { success: true });
  }
  if (method === 'PUT' && path.match(/^\/api\/users\/[^/]+\/mute$/)) {
    const id = path.split('/')[3];
    const expire = nowPlusSeconds(parseInt(body.seconds) || 0);
    await p.query('UPDATE profiles SET is_muted = 1, mute_expire_at = ? WHERE _id = ?', [expire, id]);
    return sendJson(res, 200, { success: true, mute_expire_at: expire });
  }
  if (method === 'PUT' && path.match(/^\/api\/users\/[^/]+\/unmute$/)) {
    const id = path.split('/')[3];
    await p.query('UPDATE profiles SET is_muted = 0, mute_expire_at = NULL WHERE _id = ?', [id]);
    return sendJson(res, 200, { success: true });
  }

  // ---------- 封禁状态 ----------
  if (method === 'GET' && path === '/api/ban-status') {
    const [rows] = await p.query('SELECT is_banned, ban_expire_at, is_muted, mute_expire_at FROM profiles WHERE _id = ? LIMIT 1', [query.userId]);
    ...
    if (banned && u.ban_expire_at) {
      if (new Date(u.ban_expire_at).getTime() <= now) {
        await p.query('UPDATE profiles SET is_banned = 0, ban_expire_at = NULL WHERE _id = ?', [query.userId]);
        banned = false;
      }
    }
    if (muted && u.mute_expire_at) {
      if (new Date(u.mute_expire_at).getTime() <= now) {
        await p.query('UPDATE profiles SET is_muted = 0, mute_expire_at = NULL WHERE _id = ?', [query.userId]);
        muted = false;
      }
    }
    return sendJson(res, 200, {
  ...
  if (method === 'GET' && path === '/api/dashboard') {
    const [uRows] = await p.query('SELECT COUNT(*) AS c FROM profiles');
    const today = todayStr();
    const [cRows] = await p.query('SELECT COUNT(*) AS c FROM check_ins WHERE date = ?', [today]);
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const [r] = await p.query('SELECT COUNT(*) AS c FROM check_ins WHERE date = ?', [d]);
      trend.push({ date: d, count: r[0].c });
    }

    const [pollRows] = await p.query('SELECT p._id, p.question, COUNT(v._id) AS cnt FROM polls p LEFT JOIN vote_records v ON p._id = v.pollId GROUP BY p._id ORDER BY cnt DESC LIMIT 3');

    const [rankRows] = await p.query('SELECT a.userId, a.score, a.level, p.nickname FROM user_activities a LEFT JOIN profiles p ON a.userId = p._id ORDER BY a.score DESC LIMIT 10');
    ...
    const [wRows] = await p.query('SELECT COUNT(*) AS c FROM moments WHERE created_at >= ?', [weekAgo]);
    const [mRows] = await p.query('SELECT COUNT(*) AS c FROM moments WHERE created_at >= ?', [monthAgo]);
...
module.exports = async (req, res) => {
  ...
  try {
    const fullUrl = new URL(req.url, 'https://example.com');
    const path = fullUrl.pathname;
    const query = {};
    for (const [k, v] of fullUrl.searchParams.entries()) { query[k] = v; }
    let body = {};
